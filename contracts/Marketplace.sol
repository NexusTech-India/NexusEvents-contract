...
contract Marketplace is ReentrancyGuard {
    ...
    function listBulkItems(
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            require(
                Evnt(nftAddress).ownerOf(tokenIds[i]) == msg.sender,
                "Not owner of NFT"
            );
            require(
                s_listings[nftAddress][tokenIds[i]].price == 0,
                "Already listed"
            );

            s_listings[nftAddress][tokenIds[i]] = Listing(price, msg.sender);
            emit ItemListed(msg.sender, nftAddress, tokenIds[i], price);
        }
    }
        ...
        nonReentrant
    {
        ...
        delete (s_listings[nftAddress][tokenId]);
        Evnt(nftAddress).safeTransferFrom(
            listedItem.seller,
            msg.sender,
            tokenId
        );
